...
<header class="hd" data-tauri-drag-region={p.pinned ? "false" : true}>
  ...
  <section class="content" class:rail={!sidebarOpen} data-tauri-drag-region="false">
    <aside class="picker">
      {#each p.providers as prov (prov.id)}
        ...
        <button class="px" class:active={active === prov.id} onclick={() => focus(prov.id)} style="--accent: {prov.accent};" title={providerName(prov.id, p.lang, prov.name)}>
          <span class="px-logo"><Logo id={prov.id} size={14} dark /></span>
    ...
    <div class="detail" style="--accent: {activeProvider?.accent ?? '#7a7e8a'};">
      <div class="big-row">
        <span class="big-logo"><Logo id={active} size={20} dark /></span>
      ...
      {:else if view === "status"}
        ...
        {:else}
          ...
          {@const compMap = Object.fromEntries(st.components.map(c => [c.name, c.status]))}
          ...
          <div class="sv-c-summary">
            <div class="sv-c-ring">
              <svg width="48" height="48" viewBox="0 0 48 48">
                <circle cx="24" cy="24" r="20" fill="none" stroke="#262731" stroke-width="4"/>
          ...
          {#if incidents.length > 0}
            {#each incidents.slice(0,2) as inc}
...
<style>
  ...
  .title { font-size: 20px; font-weight: 400; letter-spacing: -0.015em; color: #ebecf0; pointer-events: none; line-height: 1; }
  .date { margin-left: 14px; padding-left: 14px; border-left: 1px solid #34363f; font-size: 10px; line-height: 1.4; color: #9ca0ac; pointer-events: none; font-feature-settings: "tnum"; }
  ...
  .ct:hover { background: #262731; color: #ebecf0; }
  .ct.on { background: #ebecf0; color: #1d1e26; }
  .ct.danger:hover { background: #ebecf0; color: #1d1e26; }
  .ct.spin svg { animation: spin 0.8s linear infinite; }
  @keyframes spin { to { transform: rotate(360deg); } }
  .rule { height: 1px; background: #ebecf0; margin: 0 18px; flex-shrink: 0; }
  ...
  .picker { display: flex; flex-direction: column; gap: 4px; padding-right: 10px; border-right: 1px solid #34363f; }
  .px {
    display: flex;
    align-items: center;
    gap: 8px;
  ...
  .px .pcs { font-size: 9px; color: #5f6470; margin-left: 1px; }
  .px:hover { color: #ebecf0; }
  .px.active { color: #ebecf0; border-bottom-color: var(--accent); }
  ...
  .detail::-webkit-scrollbar-thumb { background: #34363f; }
  .big-row {
    display: flex;
  ...
  .big-name { font-size: 22px; font-weight: 200; letter-spacing: -0.02em; color: #ebecf0; line-height: 1; }
  .link {
    margin-left: auto;
  ...
  .tabs button {
    background: none; border: none; padding: 0;
    color: #5f6470; font-size: 10px;
    font-family: inherit;
    cursor: pointer;
    letter-spacing: 0.04em;
    text-transform: lowercase;
  }
  .tabs button.active { color: #ebecf0; font-weight: 500; }
  ...
  .ts-u { font-size: 9px; color: #7a7e8a; font-weight: 400; }
  .trend-meta {
    display: grid;
    grid-template-columns: repeat(4, 1fr);
    gap: 12px;
  ...
  .tm-k { font-size: 8px; letter-spacing: 0.16em; color: #7a7e8a; text-transform: uppercase; }
  .tm-v { font-size: 13px; font-weight: 500; color: #ebecf0; }
  .link:hover { text-decoration: underline; }
  .plan-line { font-size: 10px; color: #9ca0ac; padding-top: 14px; }
  .plan-line strong { color: #ebecf0; font-weight: 600; }
  .renew-sep { display: inline-block; width: 22px; }
  .hint { color: #7a7e8a; font-size: 11px; padding: 8px 0; }
  .hint.err { color: #cc3333; }
  .seg { padding: 8px 0 0 0; border-top: 1px solid #262731; margin-top: 6px; }
  .seg:first-of-type { border-top: none; }
  .seg-h { display: flex; justify-content: space-between; font-size: 9px; letter-spacing: 0.06em; color: #9ca0ac; text-transform: uppercase; }
  .seg-r { color: #5f6470; }
  .big-pc { display: flex; align-items: baseline; gap: 1px; padding: 2px 0; }
  .big-num { font-size: 28px; font-weight: 200; letter-spacing: -0.04em; color: #ebecf0; line-height: 1; font-feature-settings: "tnum"; }
  .big-sym { font-size: 12px; color: #9ca0ac; font-weight: 400; }
  .bar { height: 2px; background: #262731; position: relative; overflow: hidden; }
  .fill {
  ...
  .sv-c-pct { position: absolute; inset: 0; display: flex; align-items: center; justify-content: center; font-size: 11px; font-weight: 600; color: #aab; }
